...
"#;
    ...
    std::fs::write(&out_path, empty_code).expect("Failed to write empty maps");
...
fn generate_phf_maps(
    ...
    for (code, desc) in codes {
        output.push_str(&format!(
            ...
            code, code, desc.replace("\"", "\\\"").replace("\n", "\\n")
        ));
    }
    output.push_str("};\n\n");

    // Generate parents map
    output.push_str("pub static ICD10_CM_PARENTS: phf::Map<&'static str, Option<&'static str>> = phf_map! {\n");
    ...
    for (parent, child_list) in children {
        let children_str = child_list.iter()
            .map(|c| format!("\"{}\"", c))
            .collect::<Vec<_>>()
            .join(", ");
        output.push_str(&format!(
    ...
    std::fs::write(&out_path, output).expect("Failed to write ICD-10-CM data");
